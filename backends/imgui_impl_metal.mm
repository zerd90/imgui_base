...
@property(nonatomic, strong) id<MTLBinaryArchive>           shaderArchive;
                                    ...
                                    id<MTLRenderCommandEncoder> commandEncoder)
{
    ...
    for (int n = 0; n < drawData->CmdListsCount; n++)
    {
        ...
        memcpy((char *)vertexBuffer.buffer.contents + vertexBufferOffset, draw_list->VtxBuffer.Data,
               (size_t)draw_list->VtxBuffer.Size * sizeof(ImDrawVert));
        memcpy((char *)indexBuffer.buffer.contents + indexBufferOffset, draw_list->IdxBuffer.Data,
        ...
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            if (pcmd->UserCallback)
            {
                ...
                if (pcmd->UserCallback == ImDrawCallback_ResetRenderState)
                    ImGui_ImplMetal_SetupRenderState(drawData, commandBuffer, commandEncoder, renderPipelineState, vertexBuffer,
                                                     vertexBufferOffset);
                else
...
- (BOOL)isEqual:(id)object
{
    ...
    return other.sampleCount == self.sampleCount && other.colorPixelFormat == self.colorPixelFormat
        && other.depthPixelFormat == self.depthPixelFormat && other.stencilPixelFormat == self.stencilPixelFormat;
}
...
static ImGui::ShaderBinaryKey ImGui_ImplMetal_ShaderKey(id<MTLDevice> device)
{
    ImGui::ShaderBinaryKey key;
    key.compileTime         = ImGui::getExecutableCompileTime();
    key.systemVersion       = ImGui::getSystemVersion();
    key.gpuName             = device.name != nil ? device.name.UTF8String : "";
    NSBundle *metalBundle   = [NSBundle bundleWithIdentifier:@"com.apple.Metal"];
    NSString *driverVersion = metalBundle.infoDictionary[@"CFBundleVersion"];
    key.driverVersion       = driverVersion != nil ? driverVersion.UTF8String : "";
    return key;
}

static std::string ImGui_ImplMetal_ShaderCachePath()
{
    return (std::filesystem::u8path(ImGui::getResourcesDir()) / "metalShader.bin").u8string();
}

static id<MTLLibrary> ImGui_ImplMetal_LoadShaderLibrary(id<MTLDevice> device)
{
    NSError       *error        = nil;
    NSString      *shaderSource = [NSString stringWithUTF8String:ImGui::getShaderCode()];
...
static id<MTLBinaryArchive> ImGui_ImplMetal_LoadShaderArchive(id<MTLDevice> device, bool *loadedFromCache)
{
    *loadedFromCache = false;
    if (@available(macOS 11.0, *))
    {
        const std::string    cachePath = ImGui_ImplMetal_ShaderCachePath();
        std::vector<uint8_t> cached;
        uint32_t             cachedFormat = 0;
        if (ImGui::loadCachedShaderBinary(cachePath, ImGui_ImplMetal_ShaderKey(device), cached, cachedFormat) && !cached.empty())
        {
            NSString *tempPath = [NSTemporaryDirectory()
                stringByAppendingPathComponent:[[[NSUUID UUID] UUIDString] stringByAppendingString:@".metallib"]];
            NSURL    *tempURL  = [NSURL fileURLWithPath:tempPath];
            NSData   *data     = [NSData dataWithBytes:cached.data() length:cached.size()];
            if ([data writeToURL:tempURL atomically:YES])
            {
                NSError                    *error      = nil;
                MTLBinaryArchiveDescriptor *descriptor = [[MTLBinaryArchiveDescriptor alloc] init];
                descriptor.url                         = tempURL;
                ...
                NSLog(@"Metal shader cache rejected, recompiling: %@", error);
            }
        }
...
static void ImGui_ImplMetal_SaveShaderArchive(id<MTLBinaryArchive> archive, id<MTLDevice> device)
{
    if (archive == nil)
        return;
    if (@available(macOS 11.0, *))
    {
        ...
        {
            NSLog(@"Error: failed to serialize Metal shader archive: %@", error);
            return;
        }
        NSData *metallib = [NSData dataWithContentsOfURL:tempURL];
        [[NSFileManager defaultManager] removeItemAtURL:tempURL error:nil];
        if (metallib.length == 0)
            return;
        const std::string cachePath = ImGui_ImplMetal_ShaderCachePath();
        if (ImGui::saveCachedShaderBinary(cachePath, ImGui_ImplMetal_ShaderKey(device), metallib.bytes, metallib.length, 0))
            printf("Save %s\n", cachePath.c_str());
    }
}
                                                                   ...
                                                                   device:(id<MTLDevice>)device
{
    ...
    if (self.shaderLibrary == nil)
        self.shaderLibrary = ImGui_ImplMetal_LoadShaderLibrary(device);
    id<MTLLibrary> library = self.shaderLibrary;
    if (library == nil)
        return nil;
    ...
    if (self.shaderArchive == nil)
        self.shaderArchive = ImGui_ImplMetal_LoadShaderArchive(device, &loadedArchive);
    if (self.shaderArchive != nil && !loadedArchive)
    {
        NSError *addError = nil;
        if ([self.shaderArchive addRenderPipelineFunctionsWithDescriptor:pipelineDescriptor error:&addError])
            ImGui_ImplMetal_SaveShaderArchive(self.shaderArchive, device);
        else
            NSLog(@"Error: failed to archive Metal render pipeline: %@", addError);
    }
    if (self.shaderArchive != nil)
        pipelineDescriptor.binaryArchives = @[ self.shaderArchive ];

    id<MTLRenderPipelineState> renderPipelineState = [device newRenderPipelineStateWithDescriptor:pipelineDescriptor
                                                                                            error:&error];
    if (error != nil)
...
static id<MTLTexture> ImGui_ImplMetal_CreateImageTexture(id<MTLDevice> device, unsigned int width, unsigned int height,
...
namespace ImGui
{
    bool updateImageTexture(ImageData &image, TextureSource &texture)
    {
        ...
        for (unsigned int i = 0; i < planeCount; i++)
        {
            ...
            if (getPlaneInfo(image.format, image.width, image.height, i, &bytesPerPixel, &width, &height) != 0
                || image.plane[i] == nullptr)
            {
                dbg("invalid plane %u\n", i);
                return false;
            }
            ...
            if (nativeTexture == nil || nativeTexture.width != width || nativeTexture.height != height
                || nativeTexture.pixelFormat != pixelFormat)
            {
