...
//  2024-09-16: [Docking] Inputs: fixed an issue where a viewport destroyed while clicking would hog
//  mouse tracking and temporary lead to incorrect update of HoveredWindow. (#7971) 2024-07-08:
//  Inputs: Fixed ImGuiMod_Super being mapped to VK_APPS instead of VK_LWIN||VK_RWIN. (#7768)
...
static void ImGui_ImplWin32_UpdateMouseData()
{
    ...
    // if (is_app_focused)
    {
        ...
        if (!io.WantSetMousePos && bd->MouseTrackedArea == 0 && has_mouse_screen_pos)
        {
            ...
            // (0,0) when the mouse is on the upper-left corner of the app window) (This is the
            // position you can get with ::GetCursorPos() + ::ScreenToClient() or WM_MOUSEMOVE.)
            // Multi-viewport mode: mouse position in OS absolute coordinates (io.MousePos is (0,0)
            // when the mouse is on the upper-left of the primary monitor) (This is the position you
            // can get with ::GetCursorPos() or WM_MOUSEMOVE + ::ClientToScreen(). In theory adding
            // viewport->Pos to a client position would also be the same.)
    ...
    // heuristic.
    // - [X] Win32 backend correctly ignore viewports with the _NoInputs flag (here using
    // ::WindowFromPoint with WM_NCHITTEST + HTTRANSPARENT in WndProc does that)
    ...
    // - [X] Win32 backend correctly reports this regardless of another viewport behind focused and
...
static BOOL CALLBACK ImGui_ImplWin32_UpdateMonitors_EnumFunc(HMONITOR monitor, HDC, LPRECT, LPARAM)
{
    ...
    if (imgui_monitor.DpiScale <= 0.0f)
        return TRUE; // Some accessibility applications are declaring virtual monitors with a DPI of
                     // 0, see #7902.
...
IMGUI_IMPL_API LRESULT ImGui_ImplWin32_WndProcHandler(HWND hwnd, UINT msg, WPARAM wParam,
                                                      LPARAM lParam)
{
    // Most backends don't have silent checks like this one, but we need it because WndProc are
    // called early in CreateWindow(). We silently allow both context or just only backend data to
    // be nullptr.
    ImGui_ImplWin32_Data *bd = ImGui_ImplWin32_GetBackendData();
    if (bd == nullptr)
        return 0;
    ...
    switch (msg)
    {
        ...
        case WM_SYSKEYUP:
        {
            ...
            if (wParam < 256)
            {
                ...
                // Special behavior for VK_SNAPSHOT / ImGuiKey_PrintScreen as Windows doesn't emit
                // the key down event.
        ...
        case WM_CHAR:
            ...
            {
                wchar_t wch = 0;
                ::MultiByteToWideChar(bd->KeyboardCodePage, MB_PRECOMPOSED, (char *)&wParam, 1,
                                      &wch, 1);
...
static void ImGui_ImplWin32_CreateWindow(ImGuiViewport *viewport)
{
    ...
    vd->Hwnd      = ::CreateWindowExW(vd->DwExStyle, L"ImGui Platform", L"Untitled",
...
static void ImGui_ImplWin32_UpdateWindow(ImGuiViewport *viewport)
{
    ...
    // Unlike style settings derived from configuration flags, this is more likely to change for
    // advanced apps that are manipulating ParentViewportID manually.
    HWND new_parent = ImGui_ImplWin32_GetHwndFromViewportID(viewport->ParentViewportId);
    if (new_parent != vd->HwndParent)
    {
        // Win32 windows can either have a "Parent" (for WS_CHILD window) or an "Owner" (which among
        // other thing keeps window above its owner). Our Dear Imgui-side concept of parenting only
        // mostly care about what Win32 call "Owner". The parent parameter of CreateWindowEx() sets
        // up Parent OR Owner depending on WS_CHILD flag. In our case an Owner as we never use
        // WS_CHILD. Calling ::SetParent() here would be incorrect: it will create a full child
        // relation, alter coordinate system and clipping. Calling ::SetWindowLongPtr() with
        // GWLP_HWNDPARENT seems correct although poorly documented.
    ...
    // Generally they won't change unless configuration flags are changed, but advanced uses (such
    // as manually rewriting viewport flags) make this useful.
    DWORD new_style;
    DWORD new_ex_style;
    ImGui_ImplWin32_GetWin32StyleFromViewportFlags(viewport->Flags, &new_style, &new_ex_style);

    // Only reapply the flags that have been changed from our point of view (as other flags are
    // being modified by Windows)
