...
static NSScreen* ImGui_ImplOSX_PrimaryScreen()
{
    return NSScreen.screens.firstObject;
}

static NSWindow* ImGui_ImplOSX_MainWindow()
{
    ImGui_ImplOSX_Data* bd = ImGui_ImplOSX_GetBackendData();
    return bd != nullptr ? bd->Window : nil;
}

// Top-left origin, y down, relative to the primary screen. Matches glfwGetWindowPos/glfwGetMonitorWorkarea.
static ImRect ImGui_ImplOSX_CocoaRectToTopLeft(NSRect rect)
{
    NSScreen* screen      = ImGui_ImplOSX_PrimaryScreen();
    NSRect    screenFrame = screen != nil ? screen.frame : NSMakeRect(0, 0, rect.size.width, rect.size.height);
    float     x           = (float)(rect.origin.x - screenFrame.origin.x);
    float     y           = (float)(NSMaxY(screenFrame) - NSMaxY(rect));
    return ImRect(x, y, x + (float)rect.size.width, y + (float)rect.size.height);
}

static NSRect ImGui_ImplOSX_TopLeftToCocoaRect(const ImRect& rect)
{
    NSScreen* screen      = ImGui_ImplOSX_PrimaryScreen();
    NSRect    screenFrame = screen != nil ? screen.frame : NSMakeRect(0, 0, rect.GetWidth(), rect.GetHeight());
    return NSMakeRect(screenFrame.origin.x + rect.Min.x, NSMaxY(screenFrame) - rect.Max.y, rect.GetWidth(), rect.GetHeight());
}
...
namespace ImGui
{
    ...
    {
        NSWindow* window = ImGui_ImplOSX_MainWindow();
        if (window == nil)
            return;
        ...
        if (window.zoomed)
            [window zoom:nil];
        NSRect content = ImGui_ImplOSX_TopLeftToCocoaRect(winRect);
    ...
    ImRect getWindowRect()
    {
        ...
        return ImGui_ImplOSX_CocoaRectToTopLeft(content);
    }
