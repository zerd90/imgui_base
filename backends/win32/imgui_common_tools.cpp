...
namespace ImGui
{
    ...
    bool restartApplication(const string &scriptPath, const string &programPath)
    {
        ...
        if (!CreateProcessW(nullptr, const_cast<wchar_t *>(wideBatchPath.c_str()), nullptr, nullptr, FALSE, 0, nullptr, nullptr,
                            &si, &pi))
        {
    ...
    vector<FreetypeFontFamilyInfo> listSystemFonts()
    {
        ...
        for (UINT32 i = 0; i < familyCount; ++i)
        {
            ...
            for (UINT32 fontIdx = 0; fontIdx < fontCount; ++fontIdx)
            {
                ...
                if (FAILED(hr))
                {
                    gLastError = combineString("Failed to get font file loader for font file at index ", fontIdx, " in family ",
            ...
            if (fontInfos.size() > 0
                && std::find_if(fontFamilyInfos.begin(), fontFamilyInfos.end(), [&familyInfo](FreetypeFontFamilyInfo &existFamily)
